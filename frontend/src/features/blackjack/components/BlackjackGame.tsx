...
import {
  getPlayerDetails,
  updatePlayerBalance,
} from "../../../services/playerService"; // Adjust path if needed
...
const CardComponent: React.FC<{ card: Card | "back" }> = ({ card }) => {
  ...
  const imagePath =
    card === "back"
      ? `/images/back.png`
      : `/images/${card.suit}-${card.rank}.png`;
  return (
    <div className={styles.card}>
      <img
        src={imagePath}
        alt={card === "back" ? "Card Back" : `${card.suit}-${card.rank}`}
      />
    </div>
  );
};
...
const BlackjackGame: React.FC = () => {
  ...
  const drawCard = (
    currentDeck: Card[]
  ): { card: Card | null; updatedDeck: Card[] } => {
    if (currentDeck.length === 0) {
      console.warn("Deck empty.");
      return { card: null, updatedDeck: [] };
    }
  ...
  const startNewRound = (): void => {
    if (playerBalance === null) {
      setError("Cannot start: Balance not loaded.");
      return;
    }
    ...
    const newBetOnRoundStart = Math.max(
      MIN_BET,
      Math.min(DEFAULT_BET, playerBalance)
    );
    console.log(
      `DEBUG: startNewRound - Setting currentBet to: ${newBetOnRoundStart} (Balance: ${playerBalance})`
    );
  ...
  const dealInitialCards = (): void => {
    ...
    if (card1) pHand.push(card1);
    if (card2) dHand.push(card2);
    if (card3) pHand.push(card3);
    if (card4) dHand.push(card4);
    setDeck(currentDeck);
    setPlayerHand(pHand);
    setDealerHand(dHand);
    setGameMessage("");
  ...
  const hit = (): void => {
    ...
    if (card) {
      ...
      if (calculateHandValue(newPlayerHand) > 21) {
        determineWinner(newPlayerHand, dealerHand);
        setGameOver(true);
      }
    }
  };
  ...
  const dealerTurn = (
    ...
    let deckAfterDraws = [...currentDeck];
    ...
    while (dealerTotal < 17) {
      ...
      if (!card) break;
      hand = [...hand, card];
      dealerTotal = calculateHandValue(hand);
      deckAfterDraws = updatedDeck;
    }
    setDealerHand(hand);
    setDeck(deckAfterDraws);
    playSound("dealer-turn.mp3");
    determineWinner(currentPlayerHand, hand);
    setGameOver(true);
  };
  ...
  const determineWinner = (pHand: Card[], dHand: Card[]): void => {
    ...
    let roundMessage = "";
    let balanceChange = 0;
    const currentLockedBet = lockedBet ?? 0;
    const playerHasBlackjack = playerTotal === 21 && pHand.length === 2;
    const dealerHasBlackjack = dealerTotal === 21 && dHand.length === 2;

    if (playerHasBlackjack && dealerHasBlackjack) {
      roundMessage = "Push! Both Blackjack!";
      balanceChange = 0;
    } else if (playerHasBlackjack) {
      roundMessage = "Blackjack! You win!";
      playSound("win.wav");
      balanceChange = Math.floor(currentLockedBet * 1.5);
    } else if (dealerHasBlackjack) {
      roundMessage = "Dealer Blackjack! Dealer wins!";
      playSound("lose.wav");
      balanceChange = -currentLockedBet;
    } else if (playerTotal > 21) {
      roundMessage = "You busted! Dealer wins!";
      playSound("lose.wav");
      balanceChange = -currentLockedBet;
    } else if (dealerTotal > 21) {
      roundMessage = "Dealer busted! You win!";
      playSound("win.wav");
      balanceChange = currentLockedBet;
    ...
    if (userId && token && balanceChange !== 0) {
      ...
      console.error("Cannot update balance: Auth missing.");
      setError("Auth error.");
    }
  };
  ...
  const changeBet = (amount: number): void => {
    ...
    setCurrentBet((prev: number) =>
      Math.max(MIN_BET, Math.min(playerBalance, prev + amount))
    );
  };

  const handleSetBet = (): void => {
    if (playerBalance === null || currentBet > playerBalance) {
      setGameMessage("Insufficient balance!");
      return;
    }
    if (currentBet >= MIN_BET) {
      playSound("button-click.mp3");
      setLockedBet(currentBet);
  ...
  const handleReadyClick = (): void => {
    if (lockedBet === null) {
      setGameMessage("Please set bet first!");
      return;
    }
    playSound("button-click.mp3");
    setIsReady(true);
  ...
  const handleAutoReadyChange = (
    ...
    setIsAutoReady(e.target.checked);
    console.log("Auto Ready:", e.target.checked);
  };

  // --- Render Logic ---
  const renderPlayerSpots = () => {
    const otherPlayerSpots = [
      { id: "p1", name: "No Player", style: styles.playerOtherSpot1 },
      { id: "p2", name: "No Player", style: styles.playerOtherSpot2 },
      { id: "p3", name: "No Player", style: styles.playerOtherSpot3 },
      { id: "p4", name: "No Player", style: styles.playerOtherSpot4 },
      { id: "p5", name: "No Player", style: styles.playerOtherSpot5 },
      { id: "p6", name: "No Player", style: styles.playerOtherSpot6 },
    ];
    return (
      <>
        <div className={`${styles.playerSpot} ${styles.dealerSpot}`}>
          ...
          <div className={`${styles.handContainer} ${styles.dealerHand}`}>
            {dealerHand.map((card, index) =>
              ...
              playerHand.length > 0 &&
              dealerHand.length > 1 ? (
                <CardComponent key={`dealer-back-${index}`} card="back" />
              ) : (
                <CardComponent key={`dealer-${index}`} card={card} />
              )
            )}
          </div>
          {!gameOver && playerHand.length > 0 && dealerHand.length > 1 && (
            <span className={styles.handValue}>
              Shows: {calculateHandValue([dealerHand[1]])}
            </span>
          )}
        ...
        <div className={`${styles.playerSpot} ${styles.playerSelfSpot}`}>
          {playerName || "Player"}
          <div className={styles.balance}>
            {playerBalance !== null ? `${playerBalance}$` : "Loading..."}
          </div>
        ...
        {otherPlayerSpots.map((spot) => (
          <div key={spot.id} className={`${styles.playerSpot} ${spot.style}`}>
            {spot.name}
          </div>
        ))}
      </>
    ...
    if (userId && token && playerBalance === null && !isAuthLoading) {
      ...
      getPlayerDetails(userId, token)
        .then((data) => {
          ...
          const initialBet = Math.max(
            MIN_BET,
            Math.min(DEFAULT_BET, fetchedBalance)
          );
          console.log(
            `DEBUG: useEffect - Fetched Balance: ${fetchedBalance}, Setting initial currentBet to: ${initialBet}`
          );
  ...
  if (isAuthLoading || isLoadingData)
    return <div className={styles.gameScreen}>Loading...</div>;
  if (error && !userId)
    return (
      <div className={styles.gameScreen}>Error: {error} Please log in.</div>
    );
  if (playerBalance === null)
    return (
      <div className={styles.gameScreen}>
        Error: Could not load player balance. {error}
      </div>
    );
  ...
  const renderControls = () => {
    ...
    if (!gameOver && lockedBet === null && playerHand.length === 0) {
      ...
      console.log("DEBUG: RenderControls - Betting Phase Active", {
        ...
        disablePlus10:
          lockedBet !== null ||
          playerBalance === null ||
          currentBet + 10 > Number(playerBalance), // Explicit cast
        disablePlus50:
          lockedBet !== null ||
          playerBalance === null ||
          currentBet + 50 > Number(playerBalance), // Explicit cast
        disableSetBet:
          lockedBet !== null ||
          playerBalance === null ||
          currentBet < MIN_BET ||
          currentBet > Number(playerBalance), // Explicit cast
      });
      return (
        <>
          <div className={styles.bettingControls}>
            <div className={styles.betSection}>
              Current Bet:{" "}
              <span className={styles.betAmountDisplay}>{currentBet}$</span>
            </div>
            <div className={styles.betSection}>Change Bet:</div>
            {/* Refined disabled logic */}
            <div className={styles.betSection}>
              <button
                className={styles.betChangeButton}
                onClick={() => changeBet(-50)}
                disabled={lockedBet !== null || currentBet <= 50}
              >
                - 50
              </button>
            </div>
            <div className={styles.betSection}>
              <button
                ...
                disabled={lockedBet !== null || currentBet <= MIN_BET}
              >
                - 10
              </button>
            </div>
            ...
            <div className={styles.betSection}>
              <button
                ...
                disabled={
                  lockedBet !== null ||
                  playerBalance === null ||
                  currentBet + 10 > Number(playerBalance)
                }
              >
                + 10
              </button>
            ...
            <div className={styles.betSection}>
              <button
                ...
                disabled={
                  lockedBet !== null ||
                  playerBalance === null ||
                  currentBet + 50 > Number(playerBalance)
                }
              >
                + 50
              </button>
            ...
            <div className={styles.betSection}>
              <button
                ...
                disabled={
                  ...
                  playerBalance === null ||
                  currentBet < MIN_BET ||
                  currentBet > Number(playerBalance)
                }
              >
                Set Bet: {currentBet}$
              </button>
          ...
          <div className={styles.readyControls}>
            <span style={{ color: "#aaa" }}>Set your bet.</span>
          </div>
        </>
      );
    }
      ...
      return (
        <>
          <div className={styles.bettingControls}>
            <div className={styles.betSection}>
              Bet Placed:{" "}
              <span className={styles.betAmountDisplay}>{lockedBet}$</span>
            </div>
          </div>
          <div className={styles.readyControls}>
            <button
              ...
              disabled={isReady}
            >
              {isReady ? "Waiting..." : `Ready (${lockedBet}$)`}
            </button>
            <label>
              <input
                type="checkbox"
                checked={isAutoReady}
                onChange={handleAutoReadyChange}
              />{" "}
              Auto Ready
            </label>
          </div>
    ...
    if (!gameOver && lockedBet !== null && playerHand.length > 0) {
      return (
        <>
          ...
          <div className={styles.readyControls}>
            <button className={styles.betButton} onClick={hit}>
              Hit
            </button>
            <button className={styles.betButton} onClick={stand}>
              Stand
            </button>
          </div>
        </>
    ...
    if (gameOver) {
      return (
        <div className={styles.readyControls}>
          <button className={styles.readyButton} onClick={startNewRound}>
            Start New Game
          </button>
        </div>
      );
